...
class Recipe {
    ...
    public function search($query, $limit = 10, $offset = 0) {
        ...
        $sql = "SELECT * FROM recipes WHERE title LIKE ? ORDER BY created_at DESC LIMIT ? OFFSET ?";
        $stmt = $this->conn->prepare($sql);
    ...
    public function getById($id) {
        try {
            ...
            // Get ingredients
            $recipe['ingredients'] = getRecipeIngredients($id, $this->conn);
    ...
    private function formatRecipe($recipe) {
        ...
        if (!empty($recipe['cooking_time'])) {
            ...
            if ($hours > 0 && $minutes > 0) {
                $recipe['formatted_time'] = $hours . 'h ' . $minutes . 'm';
            } elseif ($hours > 0) {
                $recipe['formatted_time'] = $hours . 'h';
            } else {
                $recipe['formatted_time'] = $minutes . 'm';
            }
        }
